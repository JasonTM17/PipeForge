...
The `processing.job.requested` payload accepts only the four operation types implemented by the control plane: `PROFILE_DATASET`, `CHECK_MISSING_VALUES`, `CHECK_DUPLICATES`, and `DETECT_OUTLIERS`. Column identifiers are bounded SQL-style names (`[A-Za-z_][A-Za-z0-9_]{0,127}`); missing-value and duplicate checks require at least one column, while outlier detection requires `IQR` or `Z_SCORE`. Profile configuration is optional but bounded: deterministic reservoir sampling, quantiles, exact/approximate distinct strategy, common-value limits, memory budget, and sensitive-column suppression are validated before dispatch. Unknown configuration keys are rejected by the shared schema.
