...
The RabbitMQ adapter uses durable queues, persistent messages, publisher confirms, bounded prefetch, and manual acknowledgement. A command is acknowledged only after its handler returns `ACK`; malformed or unsupported commands are rejected with `requeue=false`, which hands them to the configured dead-letter exchange. The default runtime resolves lease-bound sources, executes the real pipeline, publishes attempt-scoped artifacts and lifecycle results, and consumes worker-targeted job/cancellation queues.

Worker registration and heartbeat messages are validated against the shared JSON Schemas. The worker periodically refreshes its idempotent registration with the same instance start time, so a transiently failed registry delivery can recover without a process restart. Heartbeats expose only bounded worker metadata and at most 128 opaque job IDs. Shutdown stops heartbeats, publishes `DRAINING`, drains intake, then publishes `OFFLINE` before closing the broker connection. Structured logs redact credentials, bearer tokens, and presigned URL credentials; raw dataset rows are never logged.
